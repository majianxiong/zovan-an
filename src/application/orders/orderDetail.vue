    ...
    <div class="detail marbom">
      ...
      <div class="detaillist">
        <p v-for="(item,i) in list">
          <span>{{item}}:</span>
          <i>{{detailList[i]}}<span v-if="item=='受益人'" v-show="show" v-for="(ele,j) in beneficiarys">{{ele.cname+":&nbsp;&nbsp;"+ele.benefitsRate+"%"}}<br/></span><span v-if="i==index" v-show="!show">法定</span></i>
        </p>
      </div>
    </div>
    ...
    methods:{
      getInfo(){
        ...
        .then(res=>{
          ...
          if(list.beneficiarys&&list.beneficiarys.length>0){
            this.show = true;
            this.beneficiarys = list.beneficiarys;
          }else{
            this.show=false;
          }
          if(list.proposalCode){
            this.index = 4;
          }else{
            this.index = 3;
          }
          ...
          let contentList=[list.proposalCode, organization, list.insured.cname,list.beInsured.cname,"",applyTime,effectTime,status,list.policyPlanEntities[0].amount,list.guaranteePeriod !=0?list.guaranteePeriod+"年":"终生"]
          if(!res.data.data.proposalCode){
